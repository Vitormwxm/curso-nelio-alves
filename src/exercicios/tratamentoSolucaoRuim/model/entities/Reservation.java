...
public class Reservation {
    ...
    public void updateDates(Date checkIn, Date checkOut) {
        ...
        if (checkIn.before(now) || checkOut.before(now)) {
            throw new IllegalArgumentException("Reservation dates for updates must be future"); // Illegal é erros no argumento do método
        }

        if  (!checkOut.after(checkIn)) {
            throw new IllegalArgumentException("Error in reservation: Checkout date must be after checkin date");
        }
